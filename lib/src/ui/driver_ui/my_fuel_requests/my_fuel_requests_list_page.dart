...
import 'create_fuel_token_request.dart';
...
class _MyFuelOrdersListPageState extends State<MyFuelOrdersListPage> {
  ...
  final ValueNotifier<List<FuelTokenRequest>> _inventoryItemsList = ValueNotifier<List<FuelTokenRequest>>(<FuelTokenRequest>[]);
  ...
  Widget build(BuildContext context) {
    return ListView(
      children: [
        ...
        FutureBuilder(
            future: GetIt.I<MainApiProvider>().getFuelTokenRequestsByDriverId(),
            builder: (BuildContext context, AsyncSnapshot<List<FuelTokenRequest>?> snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                _inventoryItemsList.value = List<FuelTokenRequest>.from(snapshot.data ?? <FuelTokenRequest>[]);
                    ...
                    child: SingleChildScrollView(
                      ...
                      child: Scrollbar(
                        ...
                        child: SingleChildScrollView(
                          ...
                          controller: _horizontalScrollController,
                          child: ValueListenableBuilder<List<FuelTokenRequest>>(
                                ...
                                return Padding(
                                  ...
                                  child: DataTable(
                                    ...
                                    columns: const [
                                      ...
                                      DataColumn(label: Text('Requested Quota(L)')),
                                      DataColumn(label: Text('Fuel Station Id')),
                                      DataColumn(label: Text('Scheduled Filling Date')),
  ...
  DataRow _inventoryObjectItemBuilder(BuildContext context, FuelTokenRequest data) {
    ...
    return DataRow(cells: [
      ...
      DataCell(Text(data.token == null ? "-" : "${data.token}")),
      DataCell(Text(data.requestedQuotaInLitres == null ? "-" : "${data.requestedQuotaInLitres}")),
      DataCell(Text(data.fuelStationId == null ? "-" : "${data.fuelStationId}")),
      DataCell(Text(data.scheduledFillingDate == null ? "-" : DateFormat('yyyy-MM-dd').format(data.scheduledFillingDate!))),
      DataCell(Text(data.requestedFuelType == null ? "-" : "${data.requestedFuelType?.toDisplayString()}")),
            ...
            Padding(
                ...
                child: RichText(
                  text: const TextSpan(
                      ...
                      children: [
                        TextSpan(
                            text: "My Fuel Tokens Management"
                        ),
                      ]
                  ),
                )
  ...
  void _createNewFuelOrderDialog(BuildContext context) async {
    bool isProcessSuccessful = await GeneralDialogUtils().showCustomGeneralDialog(
      context: context,
      child: const CreateFuelRequestToken(),
